...
export const getDrizzleSqliteDB = (tauriSqliteDB: Database) =>
  drizzle(
    async (sql, params, method) => {
      try {
        if (sql.startsWith('select')) {
          const selectRes = await tauriSqliteDB.select<unknown[]>(sql, params).catch((e) => {
            console.error(e);
            return [];
          });
        ...
        const res = await tauriSqliteDB.execute(sql, params);
        return { rows: [], ...res };
      } catch (e) {
        console.error(
          'Error from sqlite: ',
...
export const db = getDrizzleSqliteDB(sqliteDB);
